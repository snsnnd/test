...
#define JS_SLE_TX_CREDIT_WAIT_MS 200
...
extern uint8_t gle_tx_acb_data_num_get(void) __attribute__((weak));

static bool SLE_Wait_Tx_Credit(void)
{
    if (gle_tx_acb_data_num_get == NULL) {
        return true;
    }

    for (uint32_t wait_ms = 0; wait_ms < JS_SLE_TX_CREDIT_WAIT_MS; wait_ms++) {
...
int SLE_Send_Data(uint8_t *payload, uint16_t len) {
    ...
    if (!SLE_Wait_Tx_Credit()) {
        printf("[SLE] wait tx credit timeout len:%u\r\n", len);
        return -1;
    }
