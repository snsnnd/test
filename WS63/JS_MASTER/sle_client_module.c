...
#define JS_AUTO_SELECT_SINGLE_NODE_SEEN_COUNT 5
...
static bool g_property_notify_ready = false;
static bool g_sle_enabled = false;
static bool g_seek_started = false;
static bool g_target_found = false;
static bool g_target_connecting = false;
...
static char g_auto_select_candidate_name[SLE_NAME_MAX_LEN + 1] = {0};
static uint8_t g_auto_select_seen_count = 0;
...
static void SLE_Client_Reset_Link_State(void)
{
    ...
    g_target_connecting = false;
    g_target_connected = false;
    g_service_start_handle = 0;
    g_service_end_handle = 0;
    g_property_handle = 0;
    g_property_notify_ready = false;
    g_discovery_state = JS_DISCOVERY_IDLE;
    g_auto_select_candidate_name[0] = '\0';
    g_auto_select_seen_count = 0;
}

static void SLE_Client_Reset_Auto_Select_State(void)
{
    g_auto_select_candidate_name[0] = '\0';
    g_auto_select_seen_count = 0;
}
...
static void SLE_Client_Connect_Selected_Node(int node_index)
{
    if (node_index < 0 || node_index >= JS_DISCOVERED_NODE_MAX || !g_discovered_nodes[node_index].in_use) {
        printf("[JS Master] selected target not found:%s, keep scanning\r\n", g_target_name);
        SLE_Client_Start_Scan_Internal();
        return;
    }

    if (g_target_connecting || g_target_connected) {
        return;
    }

    g_target_found = true;
    g_target_connecting = true;
    g_target_addr = g_discovered_nodes[node_index].addr;
    printf("[JS Master] selected target:%s, prepare connect\r\n", g_target_name);
    SLE_Client_Log_Address("[JS Master] target addr:", &g_target_addr);

    if (g_seek_started) {
        SLE_Client_Log_Result("sle_stop_seek", sle_stop_seek());
    } else {
        SLE_Client_Log_Result("sle_connect_remote_device", sle_connect_remote_device(&g_target_addr));
    }
}

static bool SLE_Client_Try_Auto_Select_Single_Node(void)
{
    int single_index = -1;
    uint8_t node_count = 0;
    size_t name_len;

    if (g_target_selected || g_target_connecting || g_target_connected) {
        return false;
    }

    for (int index = 0; index < JS_DISCOVERED_NODE_MAX; index++) {
        if (!g_discovered_nodes[index].in_use || !g_discovered_nodes[index].has_service) {
            continue;
        }
        single_index = index;
        node_count++;
    ...
    if (node_count != 1 || single_index < 0) {
        SLE_Client_Reset_Auto_Select_State();
        return false;
    }

    if (strcmp(g_auto_select_candidate_name, g_discovered_nodes[single_index].name) != 0) {
        (void)snprintf(g_auto_select_candidate_name,
            sizeof(g_auto_select_candidate_name),
            "%s",
            g_discovered_nodes[single_index].name);
        g_auto_select_seen_count = 1;
        return false;
    }

    if (g_auto_select_seen_count < JS_AUTO_SELECT_SINGLE_NODE_SEEN_COUNT) {
        g_auto_select_seen_count++;
        return false;
    }

    name_len = strlen(g_discovered_nodes[single_index].name);
    if (name_len > SLE_NAME_MAX_LEN) {
        name_len = SLE_NAME_MAX_LEN;
    }
    (void)memcpy(g_target_name, g_discovered_nodes[single_index].name, name_len);
    g_target_name[name_len] = '\0';
    g_target_selected = true;

    printf("[JS Master] auto-select single target:%s after missed SELECT_TARGET\r\n", g_target_name);
    SLE_Client_Connect_Selected_Node(single_index);
    return true;
}
...
static void SLE_Client_Enable_Callback(errcode_t status)
{
    ...
    printf("[JS Master] select mode: scan and report, wait SELECT_TARGET from Atlas\r\n");
    SLE_Client_Start_Scan_Internal();
}

static void SLE_Client_Seek_Enable_Callback(errcode_t status)
{
    printf("[JS Master] scan started status:0x%lx\r\n", (unsigned long)status);
    if (status == ERRCODE_SUCC) {
        g_seek_started = true;
    }
}
...
{
    errcode_t ret;
    printf("[JS Master] scan stopped status:0x%lx selected:%u found:%u connecting:%u\r\n",
        (unsigned long)status,
        g_target_selected ? 1 : 0,
        g_target_found ? 1 : 0,
        g_target_connecting ? 1 : 0);
    g_seek_started = false;

    if ((status != ERRCODE_SUCC) || !g_target_found || !g_target_selected || !g_target_connecting) {
...
static void SLE_Client_Seek_Result_Callback(sle_seek_result_info_t *seek_result_data)
{
    ...
    if (!SLE_Adv_Data_Parse_Node(seek_result_data->data,
    ...
    SLE_Client_Record_Discovered_Node(&seek_result_data->addr,
        discovered_name,
        has_service,
        (int8_t)seek_result_data->rssi);

    if (!g_target_selected || g_target_name[0] == '\0') {
        (void)SLE_Client_Try_Auto_Select_Single_Node();
        return;
    }

    index = SLE_Client_Find_Node_By_Name(g_target_name);
    if (index < 0) {
        printf("[JS Master] discovered node:%s, current target:%s not matched\r\n",
            discovered_name[0] != '\0' ? discovered_name : "<unknown>",
            g_target_name);
        return;
    }

    if (g_target_found || g_target_connecting || g_target_connected) {
        return;
    }

    if (memcmp(g_discovered_nodes[index].addr.addr,
        seek_result_data->addr.addr,
        sizeof(seek_result_data->addr.addr)) != 0) {
        return;
    }

    printf("[JS Master] selected target matched:%s rssi:%d data_len:%u\r\n",
        g_target_name,
        (int8_t)seek_result_data->rssi,
        seek_result_data->data_length);
    ...
    sle_acb_state_t conn_state, sle_pair_state_t pair_state, sle_disc_reason_t disc_reason)
{
    ...
    if (conn_state == SLE_ACB_STATE_DISCONNECTED) {
        printf("[JS Master] peer disconnected, keep node list and wait reconnect\r\n");
    ...
    ssapc_find_property_result_t *property, errcode_t status)
{
    ...
    if (SLE_Uuid_Equals_U16(&property->uuid, JS_SLE_PROPERTY_UUID)) {
        ...
        g_property_notify_ready = (property->operate_indication & SSAP_OPERATE_INDICATION_BIT_NOTIFY) != 0;
        printf("[JS Master] 找到目标特征 handle:0x%04X operate:0x%lx descriptors:%u notify:%u\r\n",
            g_property_handle,
            (unsigned long)property->operate_indication,
            property->descriptors_count,
            g_property_notify_ready ? 1 : 0);
        for (uint8_t index = 0; index < property->descriptors_count; index++) {
            printf("[JS Master] descriptor[%u] type:0x%02X\r\n", index, property->descriptors_type[index]);
        }
    }
}
    ...
    ssapc_find_structure_result_t *structure_result, errcode_t status)
{
    ...
    if (g_discovery_state == JS_DISCOVERY_PROPERTY) {
        ...
        if (!g_property_notify_ready) {
            printf("[JS Master] target property has no notify capability\r\n");
            return;
        }
...
static void SLE_Client_Log_Packet_Preview(const char *tag, uint32_t count, ssapc_handle_value_t *data)
{
    uint16_t preview_len;

    if (tag == NULL || data == NULL || data->data == NULL) {
        return;
    }

    preview_len = data->data_len > 8 ? 8 : data->data_len;

    printf("[JS Master] %s #%lu handle:0x%04X len:%u head:",
        tag,
        (unsigned long)count,
        data->handle,
        data->data_len);
    ...
    errcode_t status)
{
    ...
    if (status != ERRCODE_SUCC || data == NULL || data->data == NULL || data->data_len == 0) {
        printf("[JS Master] notify invalid status:0x%lx\r\n", (unsigned long)status);
        return;
    }

    notify_count++;

    if (notify_count <= 20 || (notify_count % 100) == 0) {
        SLE_Client_Log_Packet_Preview("notify", notify_count, data);
    }

    if (g_notify_cb != NULL) {
        g_notify_cb(data->data, data->data_len);
    } else {
        printf("[JS Master] notify received but bridge callback is NULL\r\n");
    }
}
...
static void SLE_Client_Indication_Callback(uint8_t client_id, uint16_t conn_id, ssapc_handle_value_t *data,
    errcode_t status)
{
    static uint32_t indication_count = 0;

    (void)client_id;
    (void)conn_id;

    if (status != ERRCODE_SUCC || data == NULL || data->data == NULL || data->data_len == 0) {
        printf("[JS Master] indication invalid status:0x%lx\r\n", (unsigned long)status);
        return;
    }

    indication_count++;

    if (indication_count <= 20 || (indication_count % 100) == 0) {
        SLE_Client_Log_Packet_Preview("indication", indication_count, data);
    }

    if (g_notify_cb != NULL) {
        g_notify_cb(data->data, data->data_len);
    } else {
        printf("[JS Master] indication received but bridge callback is NULL\r\n");
    }
}
...
{
    (void)target_name;
    printf("[JS Master] select mode enabled: scan only, no auto connect\r\n");
    if (g_sle_enabled) {
        SLE_Client_Start_Scan_Internal();
    }
}
...
void SLE_Client_Select_Target(const char *target_name)
{
    ...
    if (target_name == NULL || target_name[0] == '\0') {
        printf("[JS Master] empty selected target, ignore\r\n");
        return;
    }
    ...
    printf("[JS Master] selected target received:%s\r\n", g_target_name);
    ...
    printf("[JS Master] selected target not in list yet, keep scanning:%s\r\n", g_target_name);
    if (g_sle_enabled) {
        SLE_Client_Start_Scan_Internal();
    }
}
