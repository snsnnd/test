...
#define BRIDGE_QUEUE_DEPTH              768
#define BRIDGE_QUEUE_PUT_WAIT_MS        0
...
bool Atlas_Bridge_Enqueue_Packet(const uint8_t *payload, uint16_t len)
{
    ...
    g_bridge_queue_full_count++;
    if (g_bridge_queue_full_count <= 10 || (g_bridge_queue_full_count % 100) == 0) {
        printf("[Atlas Bridge] queue full drop count=%lu len=%u\r\n",
