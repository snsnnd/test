...
static void Atlas_Bridge_Parse_Control_Stream(uint8_t *buffer, uint16_t *buffer_len, const uint8_t *input, uint16_t input_len)
{
    uint16_t write_len;
    ...
    *buffer_len = (uint16_t)(*buffer_len + write_len);
    ...
    while (*buffer_len >= sizeof(bridge_ctrl_header_t)) {
        ...
        if ((header.version == BRIDGE_CTRL_VERSION) && (header.type == BRIDGE_CTRL_TYPE_SELECT_TARGET)) {
...
static void Atlas_Bridge_Rx_Thread(void *arg)
{
    ...
    while (1) {
        ...
        read_len = uapi_uart_read(BRIDGE_UART_BUS, read_buffer, sizeof(read_buffer), BRIDGE_QUEUE_WAIT_MS);
        if (read_len > 0) {
