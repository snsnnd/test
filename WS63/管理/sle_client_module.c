...
static char g_target_name[SLE_NAME_MAX_LEN + 1] = "WS63_JS_Node_01";
    ...
    uint8_t name_out_size, bool *has_service_out)
{
    ...
    while ((data != NULL) && (offset + 2 <= data_len)) {
        ...
        if (type == JS_ADV_DATA_TYPE_COMPLETE_LIST_OF_16BIT_SERVICE_UUIDS && len >= 2) {
            uint8_t index;
            for (index = 0; (index + 1) < len; index = (uint8_t)(index + 2)) {
...
static void SLE_Client_Record_Discovered_Node(const sle_addr_t *addr, const char *name, bool has_service, int8_t rssi)
{
    ...
    if (name != NULL) {
        ...
        (void)memcpy(g_discovered_nodes[target_index].name, name, name_len);
        g_discovered_nodes[target_index].name[name_len] = '\0';
    }
}
...
static void SLE_Client_Clear_Discovered_Nodes(void)
{
    (void)memset(g_discovered_nodes, 0, sizeof(g_discovered_nodes));
}
...
static void SLE_Client_Enable_Callback(errcode_t status)
{
    ...
    printf("[JS Master] 目标从设备名称: %s\r\n", g_target_name);
    SLE_Client_Clear_Discovered_Nodes();
...
static void SLE_Client_Seek_Disable_Callback(errcode_t status)
{
    ...
    printf("[JS Master] 扫描已停止 status:0x%lx\r\n", (unsigned long)status);
    if ((status != ERRCODE_SUCC) || !g_target_found) {
...
static void SLE_Client_Seek_Result_Callback(sle_seek_result_info_t *seek_result_data)
{
    ...
    SLE_Client_Record_Discovered_Node(&seek_result_data->addr, discovered_name, has_service, (int8_t)seek_result_data->rssi);

    if ((discovered_name[0] == '\0') || (strcmp(discovered_name, g_target_name) != 0)) {
        return;
    }

    g_target_found = true;
    g_target_addr = seek_result_data->addr;
    printf("[JS Master] 命中目标广播 rssi:%d data_len:%u\r\n",
        (int8_t)seek_result_data->rssi,
        seek_result_data->data_length);
    SLE_Client_Log_Address("[JS Master] 目标地址:", &seek_result_data->addr);
    ...
    errcode_t status)
{
    printf("[JS Master] 写确认 client:%u conn:%u handle:0x%04X status:0x%lx\r\n",
        client_id,
        conn_id,
        write_result != NULL ? write_result->handle : 0,
        (unsigned long)status);
}
...
{
    return g_target_connected && (g_client_id != SLE_INVALID_CLIENT_ID) &&
        (g_conn_id != SLE_INVALID_CONN_ID) && (g_property_handle != 0);
}
...
uint8_t SLE_Client_Get_Discovered_Nodes(SLE_ClientDiscoveredNode_t *nodes, uint8_t max_nodes)
{
    ...
    for (uint8_t index = 0; index < JS_DISCOVERED_NODE_MAX && count < max_nodes; index++) {
        ...
        nodes[count].is_selected = (strcmp(g_discovered_nodes[index].name, g_target_name) == 0);
        nodes[count].is_connected = nodes[count].is_selected && g_target_connected;
        (void)memcpy(nodes[count].name, g_discovered_nodes[index].name, sizeof(nodes[count].name));
        count++;
    }
    return count;
}
...
void SLE_Client_Get_Target_State(char *target_name, uint8_t target_name_len, bool *is_connected)
{
    if ((target_name != NULL) && (target_name_len > 0)) {
        size_t copy_len = strlen(g_target_name);
        if (copy_len >= target_name_len) {
            copy_len = target_name_len - 1;
        }
        (void)memcpy(target_name, g_target_name, copy_len);
        target_name[copy_len] = '\0';
    }
